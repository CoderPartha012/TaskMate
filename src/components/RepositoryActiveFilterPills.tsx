...
export function RepositoryActiveFilterPills({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 bg-[#141420] border border-white/[0.06] rounded-lg px-3 py-2.5 mb-4">
      <span className="text-[10px] font-semibold uppercase tracking-widest text-white/35 shrink-0">Filtering by:</span>
      {pills.map((pill) => (
        <span
          key={pill.id}
          className="flex items-center gap-1.5 text-[11px] font-semibold text-white/70 bg-white/[0.06] rounded-full px-2.5 py-1"
        >
          {pill.text}
          <button
            type="button"
