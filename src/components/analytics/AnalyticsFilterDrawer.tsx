...
import {
  ...
  startOfQuarter, endOfQuarter, subQuarters, startOfYear, endOfYear,
} from 'date-fns';
...
const DATE_PRESETS: { label: string; range: () => [string, string] }[] = [
  ...
  { label: 'Last 7 days', range: () => [ymd(subDays(new Date(), 6)), ymd(new Date())] },
  { label: 'Last 30 days', range: () => [ymd(subDays(new Date(), 29)), ymd(new Date())] },
  { label: 'This month', range: () => [ymd(startOfMonth(new Date())), ymd(endOfMonth(new Date()))] },
  { label: 'Last quarter', range: () => { const lq = subQuarters(new Date(), 1); return [ymd(startOfQuarter(lq)), ymd(endOfQuarter(lq))]; } },
  { label: 'This year', range: () => [ymd(startOfYear(new Date())), ymd(endOfYear(new Date()))] },
];
            ...
            <div className="flex flex-wrap gap-1.5 mt-1.5 mb-2">
              {DATE_PRESETS.map(({ label, range }) => (
                <button
                  key={label}
                  type="button"
                  onClick={() => { const [from, to] = range(); onDraftChange({ dateFrom: from, dateTo: to }); }}
              ...
              <button
                type="button"
                onClick={() => onDraftChange({ dateFrom: '', dateTo: '' })}
                className="text-[10px] font-semibold text-white/50 hover:text-jade bg-white/[0.05] hover:bg-jade/10 rounded-full px-2.5 py-1 transition-colors"
              >
                All time
              </button>
