...
import { RepoSortKey, ColumnConfig, GroupByKey, Density, DateDisplayMode } from './repositoryColumnTypes';
...
const DENSITY_CLASSES: Record<Density, { cell: string; text: string; headCell: string }> = {
  compact:     { cell: 'px-3 py-1.5', text: 'text-[11px]', headCell: 'px-3 py-2' },
  default:     { cell: 'px-4 py-3',   text: 'text-xs',     headCell: 'px-4 py-3' },
  comfortable: { cell: 'px-4 py-4',   text: 'text-sm',     headCell: 'px-4 py-3.5' },
};
...
export function RepositoryTable({
  ...
  selectedIds, onToggleSelect, onToggleSelectAllOnPage, density, dateDisplayMode, groupByKey,
  ...
  function renderCell(task: Task, key: RepoSortKey) {
    switch (key) {
      ...
      case 'assignee':
        return <span className="truncate block">{task.assignees.length > 0 ? task.assignees.join(', ') : '—'}</span>;
      case 'createdBy':
      ...
      case 'dueDate':
        return (
          <span
            className={`whitespace-nowrap ${isOverdue(task) ? 'text-red-400 font-semibold' : ''}`}
      ...
      <table className="w-full text-left border-collapse min-w-[1150px]">
        <thead className="sticky top-0 z-10 bg-noir-800">
          <tr className="border-b-2 border-white/[0.12]">
            <th scope="col" className={`${dc.headCell} w-8`}>
              <input
                type="checkbox"
            ...
            {visibleColumns.map(({ key, label, width }) => (
              <th key={key} scope="col" className="p-0 relative" style={{ width }}>
                <button
                  type="button"
                  onClick={() => onSort(key)}
                  className={`w-full flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-white/40 hover:text-white/70 ${dc.headCell} transition-colors whitespace-nowrap`}
                >
                  {label}
                  {sortKey === key ? (
            ...
            <th scope="col" className={`text-[11px] font-semibold uppercase tracking-widest text-white/40 ${dc.headCell} text-right`}>
              Actions
            </th>
          </tr>
        </thead>
        {groups.map((group) => (
          <tbody key={group.label || 'ungrouped'}>
            ...
            {!collapsedGroups.has(group.label) && group.tasks.map((task) => {
              ...
              const borderColor = overdue ? '#EF4444' : CATEGORY_COLORS[task.taskType];
              ...
              return (
                <tr
                  ...
                  className={`border-b border-white/[0.05] last:border-b-0 hover:bg-white/[0.03] transition-colors ${dc.text} ${
                ...
                >
                  ...
                  {visibleColumns.map(({ key, width }) => (
                    <td key={key} className={`${dc.cell} text-white/45`} style={{ width, maxWidth: width }}>
                      {renderCell(task, key)}
                    </td>
                  ))}
                  <td className={dc.cell}>
