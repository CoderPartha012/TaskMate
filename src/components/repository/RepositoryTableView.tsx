...
export function RepositoryTableView({ tasks, searchTerm }: RepositoryTableViewProps) {
  ...
  const pendingHandoff = useTaskStore((s) => s.pendingRepositoryHandoff);
  ...
  const [filterDraft, setFilterDraft] = useState<RepositoryFilters>(() => pendingHandoff?.filters ?? EMPTY_REPOSITORY_FILTERS);
  const [appliedFilters, setAppliedFilters] = useState<RepositoryFilters>(() => pendingHandoff?.filters ?? EMPTY_REPOSITORY_FILTERS);
  const [quickFilter, setQuickFilter] = useState<QuickFilterKey>(() => pendingHandoff?.quickFilter ?? 'all');
  ...
  useEffect(() => {
    if (pendingHandoff) setPendingRepositoryHandoff(null);
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="text-xs font-semibold px-3 py-2 rounded-lg border border-white/[0.06] text-white/60 hover:text-white hover:border-white/[0.15] transition-colors"
            >
            ...
            <button
              ...
              className="relative flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg border border-white/[0.06] text-white/60 hover:text-white hover:border-white/[0.15] transition-colors shrink-0"
            >
