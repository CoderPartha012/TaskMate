import { AIExecutionStatus, Priority, Status, Task, TaskType } from '../../types';
...
const COUNTED_KEYS: (keyof RepositoryFilters)[] = [
  ...
  'workflowApprovalLevel', 'workflowSlaStatus', 'contractComplianceStatus', 'contractType', 'aiExecutionStatus',
];
...
function buildFilterRules(f: RepositoryFilters): FilterRule[] {
  return [
    ...
    [!!f.aiExecutionStatus, (t) => (t.aiMeta?.executionStatus ?? 'idle') === f.aiExecutionStatus],
  ];
}
