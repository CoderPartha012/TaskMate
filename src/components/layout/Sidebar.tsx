...
import { CheckCircle2, PlusCircle, Table2, BarChart2, FileDown, CalendarDays, Sparkles, Settings } from 'lucide-react';
...
const SECTIONS: { section: AppSection; icon: React.ReactNode; label: string }[] = [
  ...
  { section: 'calendar',     icon: <CalendarDays className="w-4.5 h-4.5" />, label: 'Calendar' },
  { section: 'ai-execution', icon: <Sparkles className="w-4.5 h-4.5" />,     label: 'AI Execution' },
...
export function Sidebar({ mobileOpen, onNavigate }: SidebarProps) {
  const { activeSection, setActiveSection, resetSettingsView } = useTaskStore();
          ...
          return (
            <motion.button
              ...
              onClick={() => {
                ...
                if (section === 'settings') resetSettingsView();
                onNavigate();
              }}
